...
import alpTraining as advtrain
...
def validate_architecture(architecture_name):
    ...
    assert architecture_name in ['resnet20', 'resnet32', 'resnet110', 'wide-resnet28x10'],\
...
GLOBAL_ATK_KWARGS = {'num_iterations': 10,
                     'optimizer': optim.Adam,
                     'random_init': True,
                     'optimizer_kwargs': {'lr':2.0/255.0},
                     'signed': False,
                     'verbose': False}
...
def build_fgsm_attack(classifier_net, normalizer):
    ...
    fgsm_attack = aa.FGSM(classifier_net, normalizer, delta_threat,
                           attack_loss)
    attack_kwargs = {'verbose': GLOBAL_ATK_KWARGS['verbose'], 'step_size':8.0/255.0}
...
def main(architecture_name, experiment_name, num_epochs, batch_size=256,
    ...
    train_fxn(cifar_dataset, num_epochs, criterion,
              ...
              verbosity=verbosity,regularize_adv_scale=0.2 )
